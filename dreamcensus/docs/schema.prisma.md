...
This document contains the complete Prisma schema for The Dream Census v3.
...
| Domain | Purpose |
|--------|---------|
| **Core** | Users, sessions, events |
| **Journal** | Dreams, facts, tags |
| **Census** | Instruments, sections, answers |
| **Prompts** | Daily prompts and responses |
| **Consent** | Consent ladder and receipts |
| **Weather** | Aggregates and metrics |
| **Research** | Studies and participation |
...
model CensusQuestion {
  ...
  // Question type
  type      String  // "statement", "single_choice", "multi_choice", "scale", "text", "number"
...
model Emotion {
  ...
  // Localization
  translations Json? // { "en": "Joy", "es": "Alegría", ... }
...
┌──────────────────────────────────────────────────────────────────────────────────┐
│                                    USER                                           │
│  id, displayName, email, timezone, locale, encryptionKeyVersion                  │
└────────────────────────────────────────┬─────────────────────────────────────────┘
                                         │
           ┌─────────────────────────────┼─────────────────────────────┐
           │                             │                             │
           ▼                             ▼                             ▼
┌──────────────────────┐   ┌──────────────────────┐   ┌──────────────────────┐
│       SESSION        │   │        EVENT         │   │       CONSENT        │
│  tokenHash, device   │   │  type, payload, seq  │   │  scope, granted      │
└──────────────────────┘   └──────────────────────┘   └──────────────────────┘
           ...
           ▼                             ▼                             ▼
┌──────────────────────┐   ┌──────────────────────┐   ┌──────────────────────┐
│     DREAM_ENTRY      │   │     CENSUS_ANSWER    │   │   PROMPT_RESPONSE    │
│  ciphertext, iv      │   │  questionId, value   │   │  promptId, value     │
│  emotions, vividness │   │                      │   │                      │
└──────────┬───────────┘   └──────────────────────┘   └──────────────────────┘
           │
           │
...
│                              REFERENCE DATA                                       │
├──────────────────────┬───────────────────────┬───────────────────────────────────┤
│       EMOTION        │        SYMBOL         │          THEME                    │
│  valence, arousal    │  category             │  description                      │
└──────────────────────┴───────────────────────┴───────────────────────────────────┘

┌──────────────────────────────────────────────────────────────────────────────────┐
│                              CENSUS STRUCTURE                                     │
├──────────────────────┬───────────────────────┬───────────────────────────────────┤
│  CENSUS_INSTRUMENT   │    CENSUS_SECTION     │       CENSUS_QUESTION             │
│  slug, name          │  slug, name           │  text, type, props                │
└──────────────────────┴───────────────────────┴───────────────────────────────────┘

┌──────────────────────────────────────────────────────────────────────────────────┐
│                               AGGREGATES                                          │
├─────────────────────────────────────────┬────────────────────────────────────────┤
│          WEATHER_AGGREGATE              │          PERSONAL_WEATHER              │
│  metric, period, value, sampleN         │  weather, streaks                      │
└─────────────────────────────────────────┴────────────────────────────────────────┘
```
...
-- Generated by: prisma migrate dev --name initial_schema
```

### Index Strategy

Performance-critical indexes:
- `Event(userId, type)` — Event stream queries
- `Event(sequence)` — Ordered playback
- `DreamEntry(userId, capturedAt)` — Journal list
- `JournalFact(dreamEntryId, factType)` — Fact retrieval
- `WeatherAggregate(metric, period, periodStart)` — Weather queries

### pgvector (Future)

When enabling semantic search:
...
```typescript
// Journal fact types (strongly typed)
import type { JournalFact } from '@/generated/prisma'
import type { EmotionFact, ThemeFact, SymbolFact } from '@/lib/types/facts'

type TypedJournalFact<T extends string, V> = Omit<JournalFact, 'factType' | 'value'> & {
  factType: T
  value: V
}

type EmotionJournalFact = TypedJournalFact<'emotion', EmotionFact>
type ThemeJournalFact = TypedJournalFact<'theme', ThemeFact>
type SymbolJournalFact = TypedJournalFact<'symbol', SymbolFact>
```
